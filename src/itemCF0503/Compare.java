...
//		}	
	...
	public void MAE_RMSE_recall_precision() throws ClassNotFoundException, SQLException{
		...
		pst1 = conn.prepareStatement("SELECT movieid from result1 where score>=4");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
...
/*		switch(s){
		...
		pst1 = conn.prepareStatement("SELECT sum(abs(a.score-b.score)),sum(pow(a.score-b.score,2)),count(*) from result1 a,test b where a.movieid=b.movieid and a.userid=b.userid");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			sum1 = rs1.getDouble(1);
			sum2 = rs1.getDouble(2);
