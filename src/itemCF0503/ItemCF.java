...
public class ItemCF {
	...
	public double sim_item_Euclidean(Table<Integer, Integer, Integer> itemUserRating,int item1,int item2){
		...
		SetView<Integer> comUser = Sets.intersection(set1, set2);//找到共同评分用户
...
//		    System.out.println(integer);		
		...
		for(int user:comUser){			
			score1 = userRating1.get(user);//所有所用评分之和
...
//		    System.out.println(integer);		
		...
		//通过计算两用户的共同评分项目计算相似度
		for(int user:comUser){
			sum1 += userRating1.get(user);//所有所用评分之和
			sum2 += userRating2.get(user);
			
			sum1Sq += Math.pow(userRating1.get(user), 2);//求平方之和
	...
	public List<Map.Entry<Integer, Double>> topKMatches(
		...
		double sim=0.0;
		//计算相似性
		for(int itemid:itemRating.keySet()){//item与其他userID已经评论过的项目的相似度						
			if(itemid!=itemID){
				switch(s){
					case 1:
